    ...
    <el-container style="height: 100% !important;">
      <el-main style="overflow: auto;padding: 0px;">
        ...
        <el-card v-else class="comments-card" style="padding: 0px;height: auto;">
          <div slot="header" class="comments-card-clearfix" style="height: 45px">
          ...
          <el-timeline v-if="!isEmptyValue(currentIssues)" style="padding-left: 15px;">
            ...
            >
              <span v-if="comment.issue_comment_type == 1">
                ...
                {{ messageLog(comment, true, false) }}
                <b> {{ comment.label }} </b>
                {{ messageLog(comment, false, true) }}
                <b> {{ messageLogValue(comment) }} </b>
              </span>
              <el-card v-else class="list-comments">
                <div slot="header" class="list-comments-clearfix">
                  <span v-show="!isEmptyValue(comment.user_name)">
                ...
                <div>
                  <div v-if="!comment.isEdit" v-markdown="comment.result" class="output" style="padding-left: 5px;" />
                  <span v-else>
      ...
      <el-footer style="height: 30%;padding: 0px;">
...
export default defineComponent({
  ...
  setup(props, { refs }) {
    ...
    function messageLog(comment, isChange = false, isTo = false) {
      if (
        comment.issue_comment_type === 1 &&
        isEmptyValue(comment.label) &&
        isEmptyValue(comment.displayed_value)
      ) {
        return ''
      }
      if (comment.issue_comment_type === 1 && isChange && !isTo) {
      ...
      if (
        ...
        isTo &&
        (
          !isEmptyValue(comment.new_value) || !isEmptyValue(comment.displayed_value)
        )
      ) {
    ...
    function messageLogValue(comment) {
      if (!isEmptyValue(comment.displayed_value)) {
        return comment.displayed_value
      }
      if (!isEmptyValue(comment.new_value)) {
        return comment.new_value
      }
      return ''
    }
